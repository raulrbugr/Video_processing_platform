...
int main(void)
{
	...
	//Descomentar esto para ver todas las funcionalidades de la aplicación
	DemoRun();
...
void DemoStream()
{
	...
	/*
	 * Nuevo menú de elección de filtros
	 */
	...
	while (userInput != 'q')
	{
		...
		else//Parte HW,
		{
			...
			//Esta función tiene todos los filtros integrados
 ...
 * En esta función hacemos las configuraciones de video para que se pueda lanzar todo correctamente
 */
...
void TestApp(){
	...
	//Iniciamos las capturas de imagenes y la resolución, por defecto esta 1920x1080
	if (videoCapt.state == VIDEO_DISCONNECTED) xil_printf("*Video Capture Resolution: %22s*\n\r", "<HDMI UNPLUGGED>");
	else xil_printf("*Video Capture Resolution: %17dx%-4d*\n\r", videoCapt.timing.HActiveVideo, videoCapt.timing.VActiveVideo);

	//Cambiamos la resolución de la pantalla
	status = DisplayStop(&dispCtrl);
	DisplaySetMode(&dispCtrl, &VMODE_1920x1080);
	DisplayStart(&dispCtrl);
	fResSet = 1;

	//llamamos a la operacion que lanza la ejecución
	TestApp2();
