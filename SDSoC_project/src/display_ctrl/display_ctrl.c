...
int DisplayStart(DisplayCtrl *dispPtr)
{
    ...
    */
	...
	/*
	 * TODO: This currently forces this driver to only work when the AXI stream width
	 *       is 16 bits. The "magic" 2 needs to be replaced with the width of the stream
	 *       in bytes. This could be accomplished by having it passed into the initialize
	 *       function or by having the device ID of the VDMA passed into the initialize function
	 *       so that it can be looked up using the vdmaLookupConfig function.
	 */
	dispPtr->vdmaConfig.HoriSizeInput = (dispPtr->vMode.width) * 2;
