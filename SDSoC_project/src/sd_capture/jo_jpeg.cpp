...
bool jo_write_jpg(const char *filename, const void *data, int width, int height, int comp, int quality) {
	...
	/*
	 * Write headers,, cambiar a Xilinx
	 */
	...
	/*
	 * Close the file, cambiar a Xilinx
	 */
...
bool jo_write_picture(const char *filename, u16 data[DEMO_PIXELS], int width, int height, int comp, int quality) {
	u16 pxlIn;
	///width = 1920;
	//height =1080;
	...
	f_write(&fil, std_ac_chrominance_values, sizeof(std_ac_chrominance_values),&NumBytesWritten);
	static const unsigned char head2[] = { 0xFF,0xDA,0,0xC,3,1,0,2,0x11,3,0x11,0,0x3F,0 }; //hasta aqui todo bien
		...
		for(int x = 0; x < width; x += 8) {
			...
			for(int row = y, pos = 0; row < y+8; ++row) {
				for(int col = x; col < x+8; ++col, ++pos) {
					...
					float r = ((pxlIn & 0xF800) >> (11-3));	//5 bits para el rojo
					float b = ((pxlIn & 0x07C0) >> (6-3));	//5 bits para el azul
					float g = ((pxlIn & 0x003F) << 2);		//6 bits para el verde+
...
void TakePicture(){
	...
	char *foo = new char[30*12*3]; // 4 component. RGBX format, where X is unused


	//En el vector se colocan las componentes RGB una detras de la otra,

	for(int y = 0; y < 30*3; y+=3) {

			for(int x = 0; x < 12*3; x+=3) {
 	...
 	//jo_write_jpg("foo.jpg", foo, 30, 12, 3, 90); // comp can be 1, 3, or 4. Lum, RGB, or RGBX respectively.
...
void TakePicturePress(u16 destFrame[DEMO_PIXELS], int width, int height){
		...
		xil_printf("Tomando foto\n\r");
	 	jo_write_picture("foo.jpg", destFrame, width, height, 3, 90); // comp can be 1, 3, or 4. Lum, RGB, or RGBX respectively.
	 	xil_printf("Foto tomada\n\r");
